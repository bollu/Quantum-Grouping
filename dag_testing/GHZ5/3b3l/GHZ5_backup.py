...
for item in filename_list:
    ...
    if typen == '.qasm':
        ...
        for nd in dag.topological_nodes():
            if nd.name not in all_gate_table:
                all_gate_table.append(nd.name)
                pos = len(all_gate_table) - 1
                gate_file_dict[pos] = [parentPath, counter]
        ...
        for group in finalGroupingResult:
            if len(group) <= 1:
                if dagList[group[0]][0].name not in single_gate_table:
                    single_gate_table.append(dagList[group[0]][0].name)
                continue
            ...
            for dag_id in group:
                if id_depth_dict[dag_id] % 2 == 1: # if on the first level
                    ...
                    for qarg in qargs:
                        ...
                        qasm = qasm + '[' + str(new_qarg) + ']' # for qasm
...
print(single_gate_table)

print(all_gate_table)

print(gate_file_dict)
